...
`)}function b(r){const o=h(r);s.galleryEl.innerHTML=o}let c=new m(".gallery a",{captionsData:"alt",captionPosition:"bottom",captionDelay:250});const s={formEl:document.querySelector(".form"),galleryEl:document.querySelector(".gallery"),loader:document.querySelector(".loader")};s.loader.style.display="none";s.formEl.addEventListener("submit",E);function E(r){r.preventDefault();const o=r.target.elements.text.value.trim();if(s.loader.style.display="block",!o){d();return}f(o).then(i=>{i.hits.length===0?(g(),s.galleryEl.innerHTML=""):(b(i),c.on("show.simplelightbox"),c.refresh()),s.loader.style.display="none"}).catch(i=>y(i)),r.target.reset()}
